...
class MultiRouteClient:
    ...
    def test_api_availability(self, api_name):
        ...
        try:
            if api_name == 'osrm':
                ...
                response = requests.get(self.apis['graphhopper']['url'], params=params, timeout=5)
    ...
    def get_route_osrm(self, start_lat, start_lon, end_lat, end_lon, route_type="fastest"):
        """Route via OSRM avec vraies rues"""
        try:
            if route_type == "alternative":
                # Route alternative via waypoints stratégiques de Casablanca
                waypoints = self.get_casablanca_waypoints(start_lat, start_lon, end_lat, end_lon)
    ...
    def get_route_graphhopper(self, start_lat, start_lon, end_lat, end_lon, route_type="fastest"):
        ...
        try:
            ...
            response = requests.get(self.apis['graphhopper']['url'], params=params, timeout=8)
    ...
    def get_casablanca_waypoints(self, start_lat, start_lon, end_lat, end_lon):
        ...
        if distance > 0.05:  # Trajet long
            ...
            best_point = min(strategic_points, 
    ...
    def get_best_route(self, start_lat, start_lon, end_lat, end_lon, route_type="fastest"):
        """Obtient meilleure route disponible"""
        
        # Essayer APIs dans l'ordre de préférence
        api_order = ['osrm', 'graphhopper'] if route_type == "fastest" else ['graphhopper', 'osrm']
        
        for api_name in api_order:
            if self.test_api_availability(api_name) == 'available':
                
                if api_name == 'osrm':
                    route = self.get_route_osrm(start_lat, start_lon, end_lat, end_lon, route_type)
                elif api_name == 'graphhopper':
                    route = self.get_route_graphhopper(start_lat, start_lon, end_lat, end_lon, route_type)
        ...
        return self.generate_casablanca_fallback(start_lat, start_lon, end_lat, end_lon, route_type)
    
    def generate_casablanca_fallback(self, start_lat, start_lon, end_lat, end_lon, route_type):
        """Fallback spécifique à Casablanca avec vraies rues"""
        
        # Rues principales de Casablanca (coordonnées réelles)
        main_roads = {
            'Boulevard Mohammed V': [
                (33.5731, -7.5898), (33.5750, -7.5920), (33.5780, -7.5950), (33.5820, -7.5980)
            ],
            'Avenue Hassan II': [
                (33.5600, -7.5800), (33.5650, -7.5850), (33.5700, -7.5900), (33.5731, -7.5898)
            ],
            'Boulevard Anfa': [
                (33.5650, -7.6114), (33.5680, -7.6080), (33.5720, -7.6040), (33.5750, -7.6000)
            ],
            'Boulevard Zerktouni': [
                (33.5892, -7.6031), (33.5850, -7.5980), (33.5820, -7.5940), (33.5800, -7.5900)
            ]
        }
...
VEHICLES = {
    "T-1247": {
        "id": "T-1247", "type": "🚗 Taxi", "driver": "Ahmed",
        "lat": 33.5745, "lon": -7.5890, "location": "Boulevard Mohammed V",
        "speed": 45, "color": "yellow"
    },
    "B-856": {
        "id": "B-856", "type": "🚌 Bus", "driver": "Fatima",
        "lat": 33.5765, "lon": -7.5870, "location": "Avenue Hassan II",
        "speed": 32, "color": "green"
    },
    "L-432": {
        "id": "L-432", "type": "🚛 Livraison", "driver": "Youssef",
        "lat": 33.5725, "lon": -7.5910, "location": "Boulevard Zerktouni",
        "speed": 38, "color": "red"
    },
    "V-289": {
        "id": "V-289", "type": "🚐 VTC", "driver": "Aicha",
        "lat": 33.5755, "lon": -7.5880, "location": "Centre-ville",
        "speed": 28, "color": "blue"
    }
}
...
def show_vehicles_sidebar():
    ...
    for vehicle_id, vehicle in VEHICLES.items():
        ...
        st.write(f"{status_icon} **{vehicle_id}** ({vehicle['driver']}) - {vehicle['speed']} km/h")
    ...
    with st.spinner("🌐 Calcul routes réelles via APIs..."):
        ...
        for route in routes:
            if route['api_used'] == 'osrm':
                st.success(f"🌐 {route['name']}: Route OSRM réelle (confiance: {route['confidence']:.0%})")
            elif route['api_used'] == 'graphhopper':
                st.success(f"🗺️ {route['name']}: Route GraphHopper (confiance: {route['confidence']:.0%})")
            else:
                st.info(f"🏘️ {route['name']}: Route Casablanca locale (confiance: {route['confidence']:.0%})")
...
def show_real_routes_map():
    ...
    # Ajouter véhicules
    for vehicle_id, vehicle in VEHICLES.items():
        size = 15 if st.session_state.selected_vehicle and st.session_state.selected_vehicle['id'] == vehicle_id else 10
    ...
    for i, route in enumerate(st.session_state.calculated_routes):
        ...
        with st.container():
            ...
            with col2:
                ...
                avg_speed = round((route['distance'] / route['duration']) * 60, 1) if route['duration'] > 0 else 0
            ...
            with action_col1:
                if st.button(f"🎯 Choisir Route", key=f"choose_{i}", use_container_width=True):
                    st.balloons()
                    st.success(f"🚀 Navigation activée!")
                    show_turn_by_turn_directions(route)
            
            with action_col2:
                if st.button(f"📊 Analyser", key=f"analyze_{i}", use_container_width=True):
                    show_detailed_route_analysis(route)
            
            with action_col3:
                if st.button(f"🗺️ Export GPS", key=f"export_{i}", use_container_width=True):
...
def show_detailed_route_analysis(route):
    ...
    with st.expander(f"📊 Analyse Complète - {route['name']}", expanded=True):
        ...
        with analysis_col1:
            ...
            st.write(f"**Résolution:** {len(route['coordinates'])/route['distance']:.1f} pts/km")
        ...
        if len(route['coordinates']) > 3:
            ...
            fig_speed = px.line(speed_df, x='Segment', y='Vitesse (km/h)', 
        ...
        if "Aéroport" in dest:
            recommendations.append("✈️ Destination aéroport - Prévoyez temps d'enregistrement")
        elif "Marina" in dest:
            recommendations.append("🌊 Destination Marina - Parking souvent complet weekend")
...
def show_api_monitoring():
        ...
        if st.button("🔄 Tester Toutes APIs", type="primary"):
            ...
            for api_name, result in test_results.items():
                if result['status'] == 'available':
                    ...
                    st.error(f"❌ {api_name.upper()}: {result['response_time']}ms (échec)")
    ...
    with col2:
        ...
        quality_data = pd.DataFrame({
            ...
            'Confiance (%)': [95, 90, 80],
            'Routes Calculées': [random.randint(50, 100), random.randint(20, 50), random.randint(10, 30)]
        })
        
        import plotly.express as px
        fig_quality = px.bar(quality_data, x='API', y='Confiance (%)',
        ...
        fig_perf = px.line(perf_data, x='Heure', y=['OSRM (ms)', 'GraphHopper (ms)'],
