    ...
    with col2:
        ...
        with col2a:
            if st.button("🔄 Relancer Entraînement", type="primary", key="retrain_btn"):
                with st.spinner("Entraînement en cours..."):
                    ...
                    st.success(
                        "✅ Entraînement lancé! Nouveau modèle en cours de validation"
                    )
        ...
        with col2b:
            if st.button("📊 Comparer Modèles", key="compare_btn"):
                with st.spinner("Comparaison en cours..."):
                    ...
                    st.success(
                        f"🏆 Nouveau modèle: {new_acc:.1%} vs Actuel: {current_acc:.1%}"
                    )
        ...
        pipeline_steps = {
            "Data Ingestion": {
                "status": "✅",
                "color": "green",
                "detail": "OK - 5.2k échantillons",
            },
            "Feature Engineering": {
                "status": "✅",
                "color": "green",
                "detail": "OK - 5 features",
            },
            "Model Training": {
                "status": "🔄",
                "color": "orange",
    ...
    with col3:
        ...
        if drift_score > 0.2:
            alerts.append(
                {
                    "type": "warning",
                    "message": f"Dérive données détectée (score: {drift_score:.3f})",
                }
            )

        if accuracy < 0.85:
            alerts.append(
                {"type": "error", "message": f"Performance dégradée ({accuracy:.1%})"}
            )
        else:
            alerts.append(
                {"type": "success", "message": f"Performance stable ({accuracy:.1%})"}
            )

        if outliers > 3:
            alerts.append(
                {"type": "warning", "message": f"{outliers} outliers détectés"}
            )
