    ...
    try:
        ...
        with mlflow.start_run(run_name=f"deployment_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
            ...
            for metric_name, value in metrics.items():
                if isinstance(value, (int, float)) and metric_name not in performance_metrics:
                    try:
...
def deploy_model():
    ...
    if not should_deploy:
        ...
        improvement = new_metrics.get('accuracy', 0) - (prod_metrics.get('accuracy', 0) if prod_metrics else 0)
