...
class CasablancaTrafficModelManager:
    ...
    def train_model_ensemble(self, X, y, model_type='traffic_regressor'):
        ...
        for model_name, model in self.model_configs[model_type].items():
            ...
            with mlflow.start_run(run_name=f"{model_type}_{model_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
                try:
                    # Entraînement
    ...
    def train_zone_specific_models(self, df):
        ...
        for zone in zones:
            ...
            with mlflow.start_run(run_name=f"zone_{zone}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
...
def main():
    ...
    classification_results = manager.train_model_ensemble(X, y_class, 'congestion_classifier')
